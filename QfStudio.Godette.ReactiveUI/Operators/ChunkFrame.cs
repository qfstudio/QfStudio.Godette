...
internal sealed class ChunkFrame<T>(IObservable<T> source, int frameCount, GodotFrameScheduler scheduler) : IObservable<IList<T>>
{
    ...
    private sealed class Sink : FrameSink<T, IList<T>>
    {
        ...
        public Sink(IObserver<IList<T>> observer, IObservable<T> source, int frameCount)
            : base(observer)
        {
